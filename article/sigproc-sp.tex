...
To test our application we ran the first four steps (up to but not including the final edit distance filtering pass) on a dump of current English-language Wikipedia articles (as of 5/2013) obtained from dumps.wikimedia.org. The dump is approximately 40G in size and contains 4,113,785 articles (after excluding certain non-article page types). After minhashing and deduplification, we found a total of 9,465,881 similar sentence pairs. After clustering these pairs, the total number of similar sentence clusters was 415,936, including 1,792,748 unique article/sentence pairs over 759,297 article titles. The total number of unique sentences over all cluster was 1,305,359.

Cluster sizes ranged from 2 to 28,008. (See Figure \ref{clust} for a breakdown of counts by cluster size.) Most sentence clusters are small, but the distribution of cluster exhibits a long tail. As a result, while 99\% of our clusters have size below or equal to 30, about 1/3 of the output sentence/article pairs fall into clusters with size greater than 30. One explanation for this distribution of sentences is the phenomenon we call ''tempatification,'' which we describe more in detail below.)

\subsection{Similar Sentence Types}

Sentence Clusters (without an Edit Distance Filter).
