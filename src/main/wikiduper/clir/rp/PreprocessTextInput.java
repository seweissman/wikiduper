...
public class PreprocessTextInput extends Configured implements Tool {
   ...
   */
  private static final int MinDF = 2, TermIndexWindow = 8;
  ...
  private String collectionLang = null, collectionName=null, tokenizerModel = null, collectionVocab = null, targetIndexPath = null, 
  ...
  public int run(String[] args) throws Exception {
    ...
    // Print out options
    LOG.info("Tool name: PreprocessTextInput");
    ...
    if (!fs.exists(mappingFile)) {
      ...
      String[] arr = new String[] {
          ...
          "-docnoMapping=" + mappingFile.toString()};
      LOG.info("Running GenericDocnoMappingBuilder with args " + Arrays.toString(arr));

      TextDocnoMappingBuilder tool = new TextDocnoMappingBuilder();
      tool.setConf(conf);
      tool.run(arr);

      fs.delete(new Path(indexRootPath + "/text-docid-tmp"), true);
    } else {
      LOG.info("Docno mapping already exists at: " + mappingFile);
    }
    
    // Repack into sequential compressed block
    if (!fs.exists(new Path(seqCollection + "/part-00000"))) {
      ...
      LOG.info("Running RepackText with args " + Arrays.toString(arr));

      RepackText tool = new RepackText();
      tool.setConf(conf);
      tool.run(arr);
    } else {
    ...
    conf.set(Constants.CollectionName, "europarl-"+collectionLang);
    ...
    conf.set(Constants.DocnoMappingClass, TextDocnoMapping.class.getCanonicalName());
  ...
  private static final String COLLECTION_NAME_OPTION = "collectionname";
  ...
  private int parseArgs(String[] args) {
    ...
    options.addOption(OptionBuilder.withDescription("path to input file").withArgName("path").hasArg().isRequired().create(INPUT_PATH_OPTION));
    ...
    options.addOption(OptionBuilder.withDescription("name of collection").withArgName("path").hasArg().create(COLLECTION_NAME_OPTION));
    ...
    rawCollection = cmdline.getOptionValue(INPUT_PATH_OPTION);
    ...
    collectionName = cmdline.getOptionValue(COLLECTION_NAME_OPTION);
