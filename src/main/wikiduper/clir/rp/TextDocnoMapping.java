...
/**
 * Object that maps between docids (String identifiers) to docnos (sequentially-numbered 
...
public class TextDocnoMapping implements DocnoMapping {
  private static final Logger LOG = Logger.getLogger(TextDocnoMapping.class);

  private HashMap<Integer,String> docno_docidmap = new HashMap<Integer,String>();
  private HashMap<String,Integer> docid_docnomap = new HashMap<String,Integer>();

  /**
   * Creates a {@code GenericcDocnoMapping} object.
   */
  ...
  public int getDocno(String docid) {
    ...
    return docid_docnomap.get(docid);
  }

  @Override
  public String getDocid(int docno) {
    return docno_docidmap.get(docno);
  }
  ...
  public void loadMapping(Path p, FileSystem fs) throws IOException {
    ...
    TextDocnoMapping.readMappingData(p, fs, docid_docnomap, docno_docidmap);
  }
   ...
   * @throws IOException
   */
  static public void writeMappingData(Path input, Path output, FileSystem fs, HashMap<String,Integer> docid_docnomap, HashMap<Integer, String> docno_docidmap) throws IOException {
   ...
   * @throws IOException
   */
  static public void readMappingData(Path p, FileSystem fs, HashMap<String,Integer> docid_docnomap, HashMap<Integer, String> docno_docidmap) throws IOException {
    Preconditions.checkNotNull(p);
    Preconditions.checkNotNull(fs);
    ...
    in.readInt();
    int i = 1;
    System.out.println("europarl path = " + p);
    String docid;
    try{
    while((docid = in.readUTF()) != null){
        System.out.println("docid = " + docid + ", docno = " + i + "\n");
        docid_docnomap.put(docid, i);
        docno_docidmap.put(i,docid);
        i++;
    }}catch(Exception e){
        in.close();
    }
  ...
  public Builder getBuilder() {
    return new TextDocnoMappingBuilder();
  }
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (args[0].equals("list")) {
      for (int i = 1; i < mapping.docno_docidmap.size(); i++) {
        System.out.println(i + "\t" + mapping.docno_docidmap.get(i));
      }
