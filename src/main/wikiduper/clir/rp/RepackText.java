...
/**
 * Tool for repacking Text dumps into <code>SequenceFiles</code>.
 *
 * @author Jimmy Lin
 * @author Peter Exner
 */
public class RepackText extends Configured implements Tool {
  private static final Logger LOG = Logger.getLogger(RepackText.class);

  private static enum Records { TOTAL };

  private static class MyMapper extends 
      Mapper<LongWritable, Text, IntWritable, TextDocument> {
    private static final IntWritable docno = new IntWritable();
    private static final TextDocnoMapping docnoMapping = new TextDocnoMapping();
    ...
    public void map(LongWritable key, Text doc, Context context)
      ...
      if (id != null) {
        ...
        if (n >= 0) {
          ...
          textdoc.setDocid(key.toString());
          textdoc.setContent(doc.toString());
          context.write(docno, textdoc);
        }
      }
    }
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    job.setJobName(String.format("RepackText[%s: %s, %s: %s, %s: %s, %s: %s]",
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new RepackText(), args);
  }
}
